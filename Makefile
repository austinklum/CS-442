...
sem2test:	IncrDecr forLoop funcLess constants

IncrDecr: Y
		echo "Implementation of postfix ++ and -- . Test program runs yfactors.src again but has d++ instead of d = d + 1."
		rm -f yfactorsIncr.asm		
		./Y yfactorsIncr
		cat yfactorsIncr.asm
		spim -noexception -file yfactorsIncr.asm < yfactors.in

forLoop: Y
		echo "FOR LOOP: Prints 1 through 10. Prints every odd number from 10 down to 1."
		rm -f forLoop.asm		
		./Y forLoop
		cat forLoop.asm
		spim -noexception -file forLoop.asm

funcLess: Y
		echo "PARAMATERLESS FUNCTIONS: Calls function to print a message. Calls a function that returns 5 and assigns to i. Which is the start of a for loop."
		rm -f funcLess.asm		
		./Y funcLess
		cat funcLess.asm
		spim -noexception -file funcLess.asm

constants: Y
		echo "CONSTANTS: Attempts to reassign constant j to 4,5,6,7 using several different ways. Uses j in evaluation of j + 5."
		rm -f constants.asm		
		./Y constants
		cat constants.asm
		spim -noexception -file constants.asm
...
arrays: Y
		...
		./Y arrays
		cat arrays.asm
		spim -noexception -file arrays.asm
